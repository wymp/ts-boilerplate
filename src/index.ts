 ...
 * Return helptext
 */
async function helptext(repopath: string): Promise<string> {
  const types = await getTypes(repopath);
  return `@wymp/boilerplate - Create a new project based on pre-defined boilerplate from Wymp

SYNOPSIS
    npx @wymp/boilerplate create [type] [target]
    npx @wymp/boilerplate --version
    npx @wymp/boilerplate --help|-h
 ...
 */
async function version(repopath: string): Promise<string> {
 ...
 */
async function getTypes(repopath: string): Promise<Array<string>> {
...
function exists(path: string): Promise<boolean> {
  return fs.access(path).then(_ => true).catch(e => false);
}
 ...
 */
async function copyAll(src: string, targ: string): Promise<Array<string>> {
  ...
  for (const file of files) {
    if (file.isDirectory()) {
      p.push(
        copyAll(`${src}/${file.name}`, `${targ}/${file.name}`)
        .catch(e => [e.message])
      );
    } else {
      p.push(
        fs.copyFile(`${src}/${file.name}`, `${targ}/${file.name}`)
        .then(_ => <Array<string>>[])
        .catch(e => [e.message])
      );
    }
...
async function go(argv: Array<string>) {
  ...
  while (true) {
    ...
    if (pkgExists && templatesExists) {
      ...
      const libExists = await exists(`${repopath}/lib`);
      if (libExists) {
        repopath = `${repopath}/lib`;
      } else {
